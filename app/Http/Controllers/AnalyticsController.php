...
class AnalyticsController extends Controller
{
    ...
    private function runCohortReport($analyticsData, $propertyId): array
    {
        $cohortSpec = new CohortSpec([
            'cohorts' => [
                // PERBAIKAN DI SINI: Nama cohort diubah
                new \Google\Service\AnalyticsData\Cohort(['name' => 'w0', 'dimension' => 'firstTouchDate', 'dateRange' => new GoogleDateRange(['start_date' => '7daysAgo', 'end_date' => 'today'])]),
                new \Google\Service\AnalyticsData\Cohort(['name' => 'w1', 'dimension' => 'firstTouchDate', 'dateRange' => new GoogleDateRange(['start_date' => '14daysAgo', 'end_date' => '8daysAgo'])]),
                new \Google\Service\AnalyticsData\Cohort(['name' => 'w2', 'dimension' => 'firstTouchDate', 'dateRange' => new GoogleDateRange(['start_date' => '21daysAgo', 'end_date' => '15daysAgo'])]),
                new \Google\Service\AnalyticsData\Cohort(['name' => 'w3', 'dimension' => 'firstTouchDate', 'dateRange' => new GoogleDateRange(['start_date' => '28daysAgo', 'end_date' => '22daysAgo'])]),
            ],
        ...
        foreach($retentionTable as $name => $data) {
            ...
            // Logika pencarian cohort yang benar setelah nama diubah
            $cohortsInSpec = $cohortSpec->getCohorts();
            $dateRange = null;
            foreach ($cohortsInSpec as $spec) {
                if ($spec->getName() === $name) {
                    $dateRange = $spec->getDateRange();
                    break;
                }
            }
